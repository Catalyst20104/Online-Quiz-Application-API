...
### Assumptions
- **Single-Choice Questions**: All questions are multiple-choice with one correct option. Submission expects one `selectedOptionId` per question. (Extensible for multi-select or text types.)
- **No Authentication**: Open API—no user login or quiz ownership. Assumes single-user or add JWT for production.
- **In-Memory Storage**: Data (quizzes/questions) stored in global arrays—resets on restart. Suitable for demo; assumes no concurrent users or persistence needs.
- **Option IDs**: Simple strings (e.g., "a", "b") for options. Questions use UUIDs.
- **Validation**: Basic checks (required fields, array lengths, correct option existence). No advanced sanitization (e.g., SQL injection not applicable here).
- **Error Handling**: Returns JSON errors with HTTP status codes (400/404/500). Logs to console.
- **Environment**: Local development on port 3000. No CORS (add for frontend integration).

### Design Choices
- **RESTful Architecture**: Standard HTTP methods (POST for create/add/submit, GET for fetch/list). Resource-based URLs (e.g., `/quizzes/:quizId/questions`).
- **Separation of Concerns**:
  - **Models**: Simple classes (Quiz, Question) for structure.
  - **Services**: `QuizService` handles business logic (pure functions, testable).
  - **Controllers**: Manage HTTP requests, validation, responses.
  - **Routes**: Clean endpoint mapping.
- **UUID Generation**: Uses `uuid` library for unique, collision-free IDs (better than sequential).
- **Security/Privacy**: Correct answers hidden in fetch (only server knows). No sensitive data exposed.
- **Extensibility**: Easy to add database (e.g., Mongoose), question types (e.g., `type: 'text'` with 300-char limit), or features (e.g., save scores).
- **Testing Focus**: Unit tests cover core logic (80%+ coverage). Bonus HTTP tests possible.
- **Simplicity**: No external DB or heavy deps for quick setup. In production, add dotenv for env vars, helmet for security, and a DB for persistence.

This project meets core requirements while being modular and easy to extend. For issues, check console logs or open an issue!

---

*Last Updated: 2025-10-05*  
*Author: Utkarsh Salunke*
